...
if CUSTOM_NAME:
    try:
        r = requests.get(f"https://api.oasis.com/api/v1/custom/{CUSTOM_NAME}")
        data = r.json()
        if r.ok:
            if "logo" in data:
                WEBUI_FAVICON_URL = url = (
                    f"https://api.oasis.com{data['logo']}"
                    if data["logo"][0] == "/"
                    else data["logo"]
                )
            ...
            if "splash" in data:
                url = (
                    f"https://api.oasis.com{data['splash']}"
                    if data["splash"][0] == "/"
                    else data["splash"]
                )
...
if ENV == "prod":
    if OLLAMA_BASE_URL == "/ollama" and not K8S_FLAG:
        if USE_OLLAMA_DOCKER.lower() == "true":
            # if you use all-in-one docker container (OASIS + Ollama)
            # with the docker build arg USE_OLLAMA=true (--build-arg="USE_OLLAMA=true") this only works with http://localhost:11434
            OLLAMA_BASE_URL = "http://localhost:11434"
        else:
            OLLAMA_BASE_URL = "http://host.docker.internal:11434"
    elif K8S_FLAG:
        OLLAMA_BASE_URL = "http://ollama-service.oasis.svc.cluster.local:11434"
...
ELASTICSEARCH_INDEX_PREFIX = os.environ.get(
    "ELASTICSEARCH_INDEX_PREFIX", "oasis_collections"
)
# Pgvector
...
PINECONE_INDEX_NAME = os.getenv("PINECONE_INDEX_NAME", "oasis-index")
